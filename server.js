...
async function grantLeaderboardReward(scope,rewardKey,rewardName,initialTier,source){if(!USE_SUPABASE)return{rewardKey,count:1,tier:initialTier};const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_key=eq.${encodeURIComponent(rewardKey)}&select=*`;const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);const previous=rows?.[0]||{};const count=Number(previous.award_count||0)+1;const tier=rewardTier(initialTier,count);const history=Array.isArray(previous.history)?previous.history:[];history.push({...source,awardedAt:new Date().toISOString(),tier});await supabaseRequest("/leaderboard_rewards?on_conflict=server_id,channel_id,viewer,reward_key",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:scope.serverId,channel_id:scope.channelId,viewer:scope.viewer,reward_key:rewardKey,reward_name:rewardName,reward_type:source.rewardType||"badge",award_count:count,tier,omega_count:tier==="omega"?Math.max(1,count-(initialTier==="rare"?2:initialTier==="epic"?1:0)):0,history,updated_at:new Date().toISOString()}])});return{rewardKey,count,tier};}
