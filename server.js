...
app.post("/admin/reset-backend", requireApiKey, async (req, res) => {
    ...
    if (USE_SUPABASE) {
        try {
            ...
            await supabaseRequest('/profile_cosmetics?viewer=not.is.null', { method: 'DELETE', headers: { Prefer: 'return=minimal' } }).catch(() => {});
            await supabaseRequest('/leaderboard_season_stats?viewer=not.is.null', { method: 'DELETE', headers: { Prefer: 'return=minimal' } }).catch(() => {});
            await supabaseRequest('/leaderboard_rewards?viewer=not.is.null', { method: 'DELETE', headers: { Prefer: 'return=minimal' } }).catch(() => {});
            await supabaseRequest('/leaderboard_archives?server_id=not.is.null', { method: 'DELETE', headers: { Prefer: 'return=minimal' } }).catch(() => {});
            await supabaseRequest('/leaderboard_season_config?server_id=not.is.null', { method: 'DELETE', headers: { Prefer: 'return=minimal' } }).catch(() => {});
    ...
    progressionCosmetics.clear();
    progressionNotifications.clear();
    progressionSeasonStats.clear();
    activeLeaderboardSeasons.clear();
    lastFinalizedLeaderboardMonths.clear();
...
const progressionCosmetics = new Map();
const progressionNotifications = new Map();
const progressionSeasonStats = new Map();
const activeLeaderboardSeasons = new Map();
const lastFinalizedLeaderboardMonths = new Map();
...
const PROFILE_COSMETICS = {
    frames: [
        {id:"weathered_rope",name:"Weathered Rope",tier:"common",description:"A trusty frame for every sailor.",defaultUnlocked:true},
        {id:"deckhand_canvas",name:"Deckhand Canvas",tier:"common",description:"Watch for 60 minutes.",metric:"watch_minutes",target:60},
        {id:"barnacle_iron",name:"Barnacle Iron",tier:"common",description:"Open 100 Vault Chests.",metric:"chests_opened",target:100},
        {id:"silver_tide",name:"Silver Tide",tier:"rare",description:"Earn 100 Achievement Points.",points:100},
        {id:"coral_guard",name:"Coral Guard",tier:"rare",description:"Open 1,000 Vault Chests.",metric:"chests_opened",target:1000},
        {id:"ghost_lantern",name:"Ghost Lantern",tier:"rare",description:"Complete 10 Bounties.",metric:"bounties_completed",target:10},
        {id:"enchanted_compass",name:"Enchanted Compass",tier:"epic",description:"Earn 500 Achievement Points.",points:500},
        {id:"siren_song",name:"Siren Song",tier:"epic",description:"Make 50 correct predictions.",metric:"predictions_correct",target:50},
        {id:"abyssal_runes",name:"Abyssal Runes",tier:"epic",description:"Defeat 5,000 Vault Mobs.",metric:"mobs_killed",target:5000},
        {id:"omega_kraken",name:"Omega Kraken",tier:"omega",description:"Earn 1,500 Achievement Points.",points:1500},
        {id:"celestial_navigator",name:"Celestial Navigator",tier:"omega",description:"Complete 100 Bounties.",metric:"bounties_completed",target:100},
        {id:"cursed_sovereign",name:"Cursed Sovereign",tier:"omega",description:"Earn 5,000 Achievement Points.",points:5000}
    ],
    backgrounds: [
        {id:"captains_cabin",name:"Captain's Cabin",tier:"common",description:"Warm lantern light below deck.",defaultUnlocked:true},
        {id:"quiet_harbor",name:"Quiet Harbor",tier:"common",description:"Join 5 Vaults.",metric:"vaults_joined",target:5},
        {id:"treasure_hold",name:"Treasure Hold",tier:"common",description:"Open 250 Vault Chests.",metric:"chests_opened",target:250},
        {id:"sunset_voyage",name:"Sunset Voyage",tier:"rare",description:"Complete 5 Bounties.",metric:"bounties_completed",target:5},
        {id:"moonlit_deck",name:"Moonlit Deck",tier:"rare",description:"Watch for 1,500 minutes.",metric:"watch_minutes",target:1500},
        {id:"coral_kingdom",name:"Coral Kingdom",tier:"rare",description:"Mine 250 Vault Ores.",metric:"ores_mined",target:250},
        {id:"deep_sea",name:"The Deep Sea",tier:"epic",description:"Complete 25 Bounties.",metric:"bounties_completed",target:25},
        {id:"phantom_fleet",name:"Phantom Fleet",tier:"epic",description:"Complete 50 Vaults.",metric:"vaults_completed",target:50},
        {id:"siren_grotto",name:"Siren's Grotto",tier:"epic",description:"Open 5,000 Living Chests.",metric:"living_chests",target:5000},
        {id:"omega_storm",name:"Omega Storm",tier:"omega",description:"Complete 100 Bounties.",metric:"bounties_completed",target:100},
        {id:"cosmic_ocean",name:"Cosmic Ocean",tier:"omega",description:"Earn 2,500 Achievement Points.",points:2500},
        {id:"worlds_end",name:"World's End",tier:"omega",description:"Complete 150 Vaults.",metric:"vaults_completed",target:150}
    ],
    badges: [
        {id:"crew_anchor",name:"Crew Anchor",tier:"common",icon:"⚓",description:"Join your first Vault.",metric:"vaults_joined",target:1},
        {id:"first_map",name:"First Map",tier:"common",icon:"🗺️",description:"Vote on your first quest.",metric:"votes_total",target:1},
        {id:"chest_key",name:"Chest Key",tier:"common",icon:"🗝️",description:"Open 100 Vault Chests.",metric:"chests_opened",target:100},
        {id:"bounty_skull",name:"Bounty Hunter",tier:"rare",icon:"☠️",description:"Complete 10 Bounties.",metric:"bounties_completed",target:10},
        {id:"gem_eye",name:"Gem Eye",tier:"rare",icon:"💎",description:"Mine 250 Vault Ores.",metric:"ores_mined",target:250},
        {id:"loyal_parrot",name:"Loyal Parrot",tier:"rare",icon:"🦜",description:"Watch for 1,500 minutes.",metric:"watch_minutes",target:1500},
        {id:"prophet_eye",name:"Vault Prophet",tier:"epic",icon:"🔮",description:"Make 50 correct predictions.",metric:"predictions_correct",target:50},
        {id:"kraken_mark",name:"Kraken's Mark",tier:"epic",icon:"🐙",description:"Defeat 5,000 Vault Mobs.",metric:"mobs_killed",target:5000},
        {id:"ancient_compass",name:"Ancient Compass",tier:"epic",icon:"🧭",description:"Complete 50 Vaults.",metric:"vaults_completed",target:50},
        {id:"omega_crown",name:"Omega Crown",tier:"omega",icon:"👑",description:"Earn 1,500 Achievement Points.",points:1500},
        {id:"storm_eye",name:"Eye of the Storm",tier:"omega",icon:"⚡",description:"Complete 100 Bounties.",metric:"bounties_completed",target:100},
        {id:"eternal_captain",name:"Eternal Captain",tier:"omega",icon:"🏴‍☠️",description:"Earn 5,000 Achievement Points.",points:5000},
        {id:"monthly_top5_rank1",name:"Monthly Captain #1",tier:"rare",icon:"🌙🥇",description:"Finish #1 Overall in a monthly leaderboard.",leaderboardBadge:true},
        {id:"monthly_top5_rank2",name:"Monthly First Mate #2",tier:"rare",icon:"🌙🥈",description:"Finish #2 Overall in a monthly leaderboard.",leaderboardBadge:true},
        {id:"monthly_top5_rank3",name:"Monthly Navigator #3",tier:"rare",icon:"🌙🥉",description:"Finish #3 Overall in a monthly leaderboard.",leaderboardBadge:true},
        {id:"monthly_top5_rank4",name:"Monthly Vanguard #4",tier:"rare",icon:"🌙④",description:"Finish #4 Overall in a monthly leaderboard.",leaderboardBadge:true},
        {id:"monthly_top5_rank5",name:"Monthly Vanguard #5",tier:"rare",icon:"🌙⑤",description:"Finish #5 Overall in a monthly leaderboard.",leaderboardBadge:true},
        {id:"season_top5_rank1",name:"Season Captain #1",tier:"rare",icon:"☸️🥇",description:"Finish #1 Overall at the end of a Season.",leaderboardBadge:true},
        {id:"season_top5_rank2",name:"Season First Mate #2",tier:"rare",icon:"☸️🥈",description:"Finish #2 Overall at the end of a Season.",leaderboardBadge:true},
        {id:"season_top5_rank3",name:"Season Navigator #3",tier:"rare",icon:"☸️🥉",description:"Finish #3 Overall at the end of a Season.",leaderboardBadge:true},
        {id:"season_top5_rank4",name:"Season Vanguard #4",tier:"rare",icon:"☸️④",description:"Finish #4 Overall at the end of a Season.",leaderboardBadge:true},
        {id:"season_top5_rank5",name:"Season Vanguard #5",tier:"rare",icon:"☸️⑤",description:"Finish #5 Overall at the end of a Season.",leaderboardBadge:true}
    ]
};
...
function rerollOffers(pool, seed, hasCompanion, previousChoices = []) {
    const previousIds = new Set((previousChoices || []).map(choice => choice.id));
    const freshPool = pool.filter(choice => !previousIds.has(choice.id));
    return deterministicOffers(freshPool.length >= 3 ? freshPool : pool, seed, hasCompanion);
}
function bountyRerollRules(type, used = 0) {
    const max = type === "daily" ? 1 : 2;
    const costs = type === "daily" ? [0] : [100, 250];
    return { used:Number(used||0), max, remaining:Math.max(0,max-Number(used||0)), nextCost:Number(used||0)<max?costs[Number(used||0)]:null };
}
...
function ensureProfileCosmetics(p) {
    const key=p.key||progressionKey(p.profile.serverId,p.profile.channelId,p.profile.viewer);
    if(!progressionCosmetics.has(key))progressionCosmetics.set(key,{serverId:p.profile.serverId,channelId:p.profile.channelId,viewer:p.profile.viewer,unlocked:{"frame:weathered_rope":new Date().toISOString(),"background:captains_cabin":new Date().toISOString()},equippedFrame:"weathered_rope",equippedBackground:"captains_cabin",equippedBadge:"",showcasedAchievements:[],updatedAt:new Date().toISOString()});
    const cosmetics=progressionCosmetics.get(key);evaluateCosmeticUnlocks(p,cosmetics);return cosmetics;
}
function evaluateCosmeticUnlocks(p, cosmetics) {
    const stats=p.stats.statistics||{};let changed=false;
    for(const [group,items] of Object.entries(PROFILE_COSMETICS)){const type=group==="frames"?"frame":group==="backgrounds"?"background":"badge";for(const item of items){const key=`${type}:${item.id}`;const qualified=item.defaultUnlocked||Number(p.profile.achievementPoints||0)>=Number(item.points||Infinity)||Number(stats[item.metric]||0)>=Number(item.target||Infinity);if(!qualified||cosmetics.unlocked[key])continue;cosmetics.unlocked[key]=new Date().toISOString();changed=true;emitProgressionNotification(p.profile,{kind:"cosmetic",tier:item.tier,title:"Profile Cosmetic Unlocked!",message:item.name,dedupeKey:`cosmetic:${key}`,dedupeWindowMs:31536000000,metadata:{cosmeticType:type,cosmeticId:item.id}});}}
    if(changed){cosmetics.updatedAt=new Date().toISOString();syncProfileCosmetics(cosmetics).catch(()=>{});}
}
function publicProfileCosmetics(p){const state=ensureProfileCosmetics(p);return {...state,unlocked:{...(state.unlocked||{})},showcasedAchievements:[...(state.showcasedAchievements||[])]};}
function ensureBountyState(scope, type, hasCompanion) {
    ...
    if(!state||state.periodKey!==periodKey){
        ...
        const pool=type==="daily"?DAILY_BOUNTY_POOL:WEEKLY_BOUNTY_POOL;state={...scope,periodType:type,periodKey,choices:deterministicOffers(pool,`${key}:${periodKey}`,hasCompanion),rerollCount:0,selectedBountyId:"",selectedBounty:null,progress:{},status:"choosing",rewardClaimed:false,updatedAt:new Date().toISOString()};progressionBounties.set(key,state);syncBountyState(state).catch(()=>{});
...
function bountyRewardTier(periodType,bounty={}){const reward=bounty.reward||{};if(Number(reward.ancientFragments||0)>=2)return"omega";if(Number(reward.ancientFragments||0)>=1)return"epic";if(periodType==="weekly"||Number(reward.relicFragments||0)>0)return"rare";return"common";}
function publicBountyState(state){const rules=bountyRerollRules(state.periodType,state.rerollCount);return {...state,tier:bountyRewardTier(state.periodType,state.selectedBounty||{}),progress:{...(state.progress||{})},rerollsUsed:rules.used,rerollsRemaining:rules.remaining,nextRerollCost:rules.nextCost};}
function recordProgressionMetric(scope, metric, amount=1, meta={}) {
    ...
    const stats=p.stats.statistics;stats[metric]=Math.max(0,Number(stats[metric]||0)+amount);p.stats.updatedAt=new Date().toISOString();recordSeasonMetric(scope,metric,amount);
    ...
    const pointsBefore=Number(p.profile.achievementPoints||0);evaluateAchievements(p);const pointsEarned=Number(p.profile.achievementPoints||0)-pointsBefore;if(pointsEarned>0)recordSeasonMetric(scope,"achievement_points",pointsEarned);ensureProfileCosmetics(p);syncProgressionProfile(p).catch(()=>{});
}
...
async function loadProgressionFromSupabase(){
    if(!USE_SUPABASE)return;
    try{
        const seasonConfigs=await supabaseRequest("/leaderboard_season_config?select=*",{method:"GET"}).catch(()=>[]);
        for(const row of seasonConfigs||[])if(row.server_id){const id=normalizeServerId(row.server_id);if(row.current_season_name)activeLeaderboardSeasons.set(id,String(row.current_season_name));if(row.last_finalized_month)lastFinalizedLeaderboardMonths.set(id,String(row.last_finalized_month));}
        const [profiles,stats,bounties,achievements,titles,notifications,cosmetics,seasonStats]=await Promise.all([
            ...
            supabaseRequest("/profile_cosmetics?select=*",{method:"GET"}).catch(()=>[]),
            supabaseRequest("/leaderboard_season_stats?select=*",{method:"GET"}).catch(()=>[])
        ]);
        ...
        for(const row of bounties||[]){const scope={serverId:row.server_id,channelId:row.channel_id,viewer:row.viewer};progressionBounties.set(`${progressionKey(scope.serverId,scope.channelId,scope.viewer)}::${row.period_type}`,{...scope,periodType:row.period_type,periodKey:row.period_key,choices:row.choices||[],rerollCount:Number(row.reroll_count||0),selectedBountyId:row.selected_bounty_id||"",selectedBounty:row.selected_bounty||null,progress:row.progress||{},status:row.status||"choosing",rewardClaimed:!!row.reward_claimed,selectedAt:row.selected_at,completedAt:row.completed_at,claimedAt:row.claimed_at,updatedAt:row.updated_at});}
        ...
        for(const row of cosmetics||[]){const key=progressionKey(row.server_id,row.channel_id,row.viewer);progressionCosmetics.set(key,{serverId:row.server_id,channelId:row.channel_id,viewer:row.viewer,unlocked:row.unlocked||{},equippedFrame:row.equipped_frame||"weathered_rope",equippedBackground:row.equipped_background||"captains_cabin",equippedBadge:row.equipped_badge||"",showcasedAchievements:Array.isArray(row.showcased_achievements)?row.showcased_achievements:[],updatedAt:row.updated_at});}
        for(const row of seasonStats||[]){const scope={serverId:row.server_id,channelId:row.channel_id,viewer:row.viewer};progressionSeasonStats.set(seasonStatsKey(scope,row.season_key),{...scope,seasonKey:row.season_key,statistics:row.statistics||{},updatedAt:row.updated_at});}
...
async function bountyHistoryForScope(scope,limit=30){if(!USE_SUPABASE)return[];const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&completed=eq.true&order=completed_at.desc&limit=${Math.max(1,Math.min(100,Number(limit||30)))}`;const rows=await supabaseRequest(`/bounty_history?${filter}`);return(rows||[]).map(row=>({periodType:row.period_type,periodKey:row.period_key,bountyId:row.bounty_id,bounty:row.bounty_data||{},progress:row.final_progress||{},completed:!!row.completed,rewardClaimed:!!row.reward_claimed,reward:row.reward_data||{},tier:row.bounty_data?.tier||bountyRewardTier(row.period_type,row.bounty_data||{}),selectedAt:row.selected_at,completedAt:row.completed_at,claimedAt:row.claimed_at}));}
...
async function syncAchievementBatch(profile,items){if(!USE_SUPABASE||!items.length)return;const now=new Date().toISOString();await supabaseRequest("/achievements?on_conflict=server_id,channel_id,viewer,achievement_id,tier",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify(items.map(a=>({server_id:profile.serverId,channel_id:profile.channelId,viewer:profile.viewer,achievement_id:a.achievementId,tier:a.tier,progress:a.progress,target:a.target,unlocked:a.unlocked,reward_claimed:a.rewardClaimed,reward_data:{},unlocked_at:a.unlockedAt,updated_at:now})))});}
...
async function syncTitleBatch(profile,items){if(!USE_SUPABASE||!items.length)return;const now=new Date().toISOString();await supabaseRequest("/titles?on_conflict=server_id,channel_id,viewer,title_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify(items.map(t=>({server_id:profile.serverId,channel_id:profile.channelId,viewer:profile.viewer,title_id:t.titleId,title_name:t.titleName,source_achievement_id:t.sourceAchievementId||null,unlocked_at:t.unlockedAt,updated_at:now})))});}
...
async function syncProfileCosmetics(c){if(!USE_SUPABASE)return;await supabaseRequest("/profile_cosmetics?on_conflict=server_id,channel_id,viewer",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:c.serverId,channel_id:c.channelId,viewer:c.viewer,unlocked:c.unlocked||{},equipped_frame:c.equippedFrame||"weathered_rope",equipped_background:c.equippedBackground||"captains_cabin",equipped_badge:c.equippedBadge||null,showcased_achievements:c.showcasedAchievements||[],updated_at:new Date().toISOString()}])});}
async function syncSeasonStats(s){if(!USE_SUPABASE)return;await supabaseRequest("/leaderboard_season_stats?on_conflict=server_id,channel_id,viewer,season_key",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:s.serverId,channel_id:s.channelId,viewer:s.viewer,season_key:s.seasonKey,statistics:s.statistics||{},updated_at:new Date().toISOString()}])});}
function companionMilestones(scope,profile={},stats={}){const notices=notificationList(scope);const firstNotice=kind=>notices.filter(item=>item.kind===kind).sort((a,b)=>Date.parse(a.createdAt)-Date.parse(b.createdAt))[0];const result=[];const add=(id,title,description,icon,tier="common",earnedAt="",stamp="LOGGED")=>result.push({id,title,description,icon,tier,earnedAt:earnedAt||profile.profileCreatedAt||stats.first_activity_at||"",stamp});if(profile.companionName)add("joined","Joined the Crew",`${profile.companionName} began their voyage with ${profile.displayName||profile.viewer}.`,"⚓","common",profile.profileCreatedAt,"CREW");if(Number(stats.vaults_joined||0)>0)add("first_vault","First Vault",`${profile.companionName||"This Meowty"} joined their first Vault.`,"⛵","common",stats.first_activity_at,"FIRST");if(Number(stats.vaults_completed||0)>=10)add("vault_veteran","Vault Veteran",`Completed ${Number(stats.vaults_completed).toLocaleString()} Vaults.`,"🏆","rare",stats.last_activity_at,"VETERAN");const bounty=firstNotice("bounty_claimed");if(Number(stats.bounties_completed||0)>0)add("first_bounty","First Bounty",`Completed the first Crew bounty.`,"📜","common",bounty?.createdAt,"BOUNTY");if(Number(stats.bounties_completed||0)>=10)add("bounty_hunter","Bounty Hunter",`Completed ${Number(stats.bounties_completed).toLocaleString()} bounties.`,"🎯","rare",bounty?.createdAt,"HUNTER");for(const notice of notices.filter(item=>item.kind==="slot_unlock")){const type=String(notice.metadata?.slotType||"relic");const level=Number(notice.metadata?.unlockLevel||0);const id=`${type}_slot_${level}`;if(result.some(item=>item.id===id))continue;const title=type==="trail"?"Trail Slot Unlocked":type==="ancient"?"Ancient Relic Awakened":"Relic Slot Unlocked";const icon=type==="trail"?"✨":type==="ancient"?"🔮":"💎";add(id,title,`Unlocked at Meowty Level ${level}.`,icon,type==="ancient"?"epic":level>=9?"rare":"common",notice.createdAt,"UNLOCKED");}if(Number(stats.academy_spars||0)>0)add("academy_debut","Academy Debut",`Entered Academy sparring for the first time.`,"⚔️","common",stats.first_activity_at,"ACADEMY");if(Number(profile.achievementPoints||0)>=500)add("legend_points","Crew Legend",`Earned ${Number(profile.achievementPoints).toLocaleString()} Achievement Points.`,"👑",Number(profile.achievementPoints)>=2500?"omega":Number(profile.achievementPoints)>=1000?"epic":"rare",profile.updatedAt,"LEGEND");return result.sort((a,b)=>Date.parse(b.earnedAt||0)-Date.parse(a.earnedAt||0));}

app.get("/profile/:viewer",async(req,res)=>{const scope=progressionScope(req,req.params.viewer);if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});const wallet=getWalletResolved(scopedViewerKey(scope.viewer,scope.channelId,scope.serverId),false);const linked=parseCompanionLink(wallet?.companionName||"");const p=ensureProgression(scope,wallet?.displayName||scope.viewer,linked.companionName||"");await syncLeaderboardRewardBadges(scope);const daily=ensureBountyState(scope,"daily",!!p.profile.companionName);const weekly=ensureBountyState(scope,"weekly",!!p.profile.companionName);let bountyHistory=[];try{bountyHistory=await bountyHistoryForScope(scope,30);}catch(error){console.warn("[BOUNTIES] Could not load history:",error?.message||error);}res.set("Cache-Control","no-store");res.json({ok:true,profile:p.profile,statistics:p.stats.statistics,achievements:p.achievements,titles:p.titles,achievementDefinitions:ACHIEVEMENT_DEFINITIONS,cosmeticCatalog:PROFILE_COSMETICS,cosmetics:publicProfileCosmetics(p),milestones:companionMilestones(scope,p.profile,p.stats.statistics),daily:publicBountyState(daily),weekly:publicBountyState(weekly),bountyHistory});});
app.get("/profiles/leaderboard/public",async(req,res)=>{const serverId=normalizeServerId(req.query.serverId||firstEnabledServerId());const channelId=normalizeChannelId(req.query.channelId||req.query.channel||"");if(!channelId)return res.status(400).json({ok:false,error:"Missing channel"});const entries=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(profile.serverId,profile.channelId,profile.viewer);const titles=progressionTitles.get(key)||{};const achievements=progressionAchievements.get(key)||{};const cosmetics=progressionCosmetics.get(key)||{equippedFrame:"weathered_rope",equippedBackground:"captains_cabin",equippedBadge:"",showcasedAchievements:[]};const selectedTitle=titles[profile.selectedTitleId||""];const showcased=(cosmetics.showcasedAchievements||[]).slice(0,3).map(id=>{const achievement=achievements[id]||{};const baseId=id.split(":")[0];const definition=ACHIEVEMENT_DEFINITIONS.find(item=>item.id===baseId);return{id,name:achievement.name||definition?.name||baseId.replaceAll("_"," "),tier:achievement.tier||id.split(":")[1]||"common"};});entries.push({viewer:profile.viewer,displayName:profile.displayName||profile.viewer,companionName:profile.companionName||"",selectedTitle:selectedTitle?.titleName||"",achievementPoints:Number(profile.achievementPoints||0),cosmetics:{equippedFrame:cosmetics.equippedFrame||"weathered_rope",equippedBackground:cosmetics.equippedBackground||"captains_cabin",equippedBadge:cosmetics.equippedBadge||"",showcasedAchievements:showcased}});}entries.sort((a,b)=>b.achievementPoints-a.achievementPoints||a.displayName.localeCompare(b.displayName));res.set("Cache-Control","no-store");res.json({ok:true,entries:entries.slice(0,100),cosmeticCatalog:PROFILE_COSMETICS,updatedAt:new Date().toISOString()});});
app.get("/profiles/public/:viewer",async(req,res)=>{const scope=progressionScope(req,req.params.viewer);if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});const key=progressionKey(scope.serverId,scope.channelId,scope.viewer);const profile=progressionProfiles.get(key);if(!profile)return res.status(404).json({ok:false,error:"Crew profile not found"});await syncLeaderboardRewardBadges(scope);const titles=progressionTitles.get(key)||{};const achievements=progressionAchievements.get(key)||{};const stats=progressionStats.get(key)?.statistics||{};const cosmetics=progressionCosmetics.get(key)||{equippedFrame:"weathered_rope",equippedBackground:"captains_cabin",equippedBadge:"",showcasedAchievements:[]};const selectedTitle=titles[profile.selectedTitleId||""];const ranked=Array.from(progressionProfiles.values()).filter(item=>normalizeServerId(item.serverId)===scope.serverId&&normalizeChannelId(item.channelId)===scope.channelId).sort((a,b)=>Number(b.achievementPoints||0)-Number(a.achievementPoints||0)||String(a.displayName||a.viewer).localeCompare(String(b.displayName||b.viewer)));const rank=Math.max(1,ranked.findIndex(item=>normalizeViewer(item.viewer)===scope.viewer)+1);const publicAchievements={};for(const [id,item] of Object.entries(achievements))publicAchievements[id]={unlocked:!!item.unlocked,tier:item.tier||id.split(":")[1]||"common",name:item.name||""};const showcased=(cosmetics.showcasedAchievements||[]).slice(0,3).map(id=>{const item=achievements[id]||{};const baseId=id.split(":")[0];const definition=ACHIEVEMENT_DEFINITIONS.find(value=>value.id===baseId);return{id,name:item.name||definition?.name||baseId.replaceAll("_"," "),tier:item.tier||id.split(":")[1]||"common"};});res.set("Cache-Control","no-store");res.json({ok:true,rank,profile:{viewer:profile.viewer,displayName:profile.displayName||profile.viewer,companionName:profile.companionName||"",achievementPoints:Number(profile.achievementPoints||0),selectedTitle:selectedTitle?.titleName||""},statistics:stats,achievements:publicAchievements,milestones:companionMilestones(scope,profile,stats),achievementDefinitions:ACHIEVEMENT_DEFINITIONS,cosmeticCatalog:PROFILE_COSMETICS,cosmetics:{equippedFrame:cosmetics.equippedFrame||"weathered_rope",equippedBackground:cosmetics.equippedBackground||"captains_cabin",equippedBadge:cosmetics.equippedBadge||"",showcasedAchievements:showcased}});});

function leaderboardCategoryScore(category,profile,stats={}){if(category==="bounties")return Number(stats.bounties_completed||0);if(category==="vaults")return Number(stats.vaults_completed||0);if(category==="explorer")return Number(stats.chests_opened||0)+Number(stats.ores_mined||0)+Number(stats.mobs_killed||0);if(category==="watchtime")return Number(stats.watch_minutes||0);if(category==="academy")return Number(stats.academy_spars||0);return Number(profile.achievementPoints||0);}
function leaderboardCategoryLabel(category){return({overall:"Achievement Points",bounties:"Bounties Completed",vaults:"Vaults Completed",explorer:"Explorer Score",watchtime:"Minutes Watched",academy:"Academy Spars"})[category]||"Achievement Points";}
app.use("/profiles/leaderboard/period",async(req,res,next)=>{const serverId=normalizeServerId(req.query.serverId||firstEnabledServerId());await ensureMonthlyLeaderboardFinalized(serverId).catch(error=>console.warn("[LEADERBOARD] Monthly finalization failed:",error?.message||error));next();});
app.get("/profiles/leaderboard/categories",async(req,res)=>{const serverId=normalizeServerId(req.query.serverId||firstEnabledServerId());const channelId=normalizeChannelId(req.query.channelId||req.query.channel||"");if(!channelId)return res.status(400).json({ok:false,error:"Missing channel"});const requested=String(req.query.category||"overall").toLowerCase();const category=["overall","bounties","vaults","explorer","watchtime","academy"].includes(requested)?requested:"overall";const entries=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(profile.serverId,profile.channelId,profile.viewer);const stats=progressionStats.get(key)?.statistics||{};const titles=progressionTitles.get(key)||{};const achievements=progressionAchievements.get(key)||{};const cosmetics=progressionCosmetics.get(key)||{equippedFrame:"weathered_rope",equippedBackground:"captains_cabin",equippedBadge:"",showcasedAchievements:[]};const selectedTitle=titles[profile.selectedTitleId||""];const showcased=(cosmetics.showcasedAchievements||[]).slice(0,3).map(id=>{const achievement=achievements[id]||{};const baseId=id.split(":")[0];const definition=ACHIEVEMENT_DEFINITIONS.find(item=>item.id===baseId);return{id,name:achievement.name||definition?.name||baseId.replaceAll("_"," "),tier:achievement.tier||id.split(":")[1]||"common"};});entries.push({viewer:profile.viewer,displayName:profile.displayName||profile.viewer,companionName:profile.companionName||"",selectedTitle:selectedTitle?.titleName||"",achievementPoints:Number(profile.achievementPoints||0),score:leaderboardCategoryScore(category,profile,stats),scoreLabel:leaderboardCategoryLabel(category),cosmetics:{equippedFrame:cosmetics.equippedFrame||"weathered_rope",equippedBackground:cosmetics.equippedBackground||"captains_cabin",equippedBadge:cosmetics.equippedBadge||"",showcasedAchievements:showcased}});}entries.sort((a,b)=>b.score-a.score||b.achievementPoints-a.achievementPoints||a.displayName.localeCompare(b.displayName));res.set("Cache-Control","no-store");res.json({ok:true,category,season:"all_time",scoreLabel:leaderboardCategoryLabel(category),entries:entries.slice(0,100),cosmeticCatalog:PROFILE_COSMETICS,updatedAt:new Date().toISOString()});});
app.get("/profiles/leaderboard/seasonal",async(req,res)=>{const serverId=normalizeServerId(req.query.serverId||firstEnabledServerId());const channelId=normalizeChannelId(req.query.channelId||req.query.channel||"");if(!channelId)return res.status(400).json({ok:false,error:"Missing channel"});const requested=String(req.query.category||"overall").toLowerCase();const category=["overall","bounties","vaults","explorer","watchtime","academy"].includes(requested)?requested:"overall";const seasonal=String(req.query.season||"all_time")==="current";const seasonKey=monthlySeasonKey();const entries=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(profile.serverId,profile.channelId,profile.viewer);const stats=seasonal?(progressionSeasonStats.get(`${key}::${seasonKey}`)?.statistics||{}):(progressionStats.get(key)?.statistics||{});const scoreProfile=seasonal?{achievementPoints:Number(stats.achievement_points||0)}:profile;const titles=progressionTitles.get(key)||{};const achievements=progressionAchievements.get(key)||{};const cosmetics=progressionCosmetics.get(key)||{equippedFrame:"weathered_rope",equippedBackground:"captains_cabin",equippedBadge:"",showcasedAchievements:[]};const selectedTitle=titles[profile.selectedTitleId||""];const showcased=(cosmetics.showcasedAchievements||[]).slice(0,3).map(id=>{const achievement=achievements[id]||{};const baseId=id.split(":")[0];const definition=ACHIEVEMENT_DEFINITIONS.find(item=>item.id===baseId);return{id,name:achievement.name||definition?.name||baseId.replaceAll("_"," "),tier:achievement.tier||id.split(":")[1]||"common"};});entries.push({viewer:profile.viewer,displayName:profile.displayName||profile.viewer,companionName:profile.companionName||"",selectedTitle:selectedTitle?.titleName||"",achievementPoints:Number(profile.achievementPoints||0),score:leaderboardCategoryScore(category,scoreProfile,stats),scoreLabel:seasonal&&category==="overall"?"Season Points":leaderboardCategoryLabel(category),cosmetics:{equippedFrame:cosmetics.equippedFrame||"weathered_rope",equippedBackground:cosmetics.equippedBackground||"captains_cabin",equippedBadge:cosmetics.equippedBadge||"",showcasedAchievements:showcased}});}entries.sort((a,b)=>b.score-a.score||b.achievementPoints-a.achievementPoints||a.displayName.localeCompare(b.displayName));res.set("Cache-Control","no-store");res.json({ok:true,category,season:seasonal?seasonKey:"all_time",seasonLabel:seasonal?new Intl.DateTimeFormat("en",{timeZone:"Europe/Paris",month:"long",year:"numeric"}).format(new Date()):"All Time",entries:entries.slice(0,100),cosmeticCatalog:PROFILE_COSMETICS,updatedAt:new Date().toISOString()});});

app.get("/profiles/leaderboard/period",async(req,res)=>{const serverId=normalizeServerId(req.query.serverId||firstEnabledServerId());const channelId=normalizeChannelId(req.query.channelId||req.query.channel||"");if(!channelId)return res.status(400).json({ok:false,error:"Missing channel"});const category=["overall","bounties","vaults","explorer","watchtime","academy"].includes(String(req.query.category||"overall"))?String(req.query.category||"overall"):"overall";const period=["monthly","season","all_time"].includes(String(req.query.period||"all_time"))?String(req.query.period||"all_time"):"all_time";const periodKey=period==="monthly"?monthlySeasonKey():period==="season"?`season:${configuredSeasonName(serverId)}`:"";const entries=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(profile.serverId,profile.channelId,profile.viewer);const stats=period==="all_time"?(progressionStats.get(key)?.statistics||{}):(progressionSeasonStats.get(`${key}::${periodKey}`)?.statistics||{});const scoreProfile=period==="all_time"?profile:{achievementPoints:Number(stats.achievement_points||0)};const titles=progressionTitles.get(key)||{};const achievements=progressionAchievements.get(key)||{};const cosmetics=progressionCosmetics.get(key)||{};const selectedTitle=titles[profile.selectedTitleId||""];const showcased=(cosmetics.showcasedAchievements||[]).slice(0,3).map(id=>{const a=achievements[id]||{};const base=id.split(":")[0];const def=ACHIEVEMENT_DEFINITIONS.find(x=>x.id===base);return{id,name:a.name||def?.name||base.replaceAll("_"," "),tier:a.tier||id.split(":")[1]||"common"};});entries.push({viewer:profile.viewer,displayName:profile.displayName||profile.viewer,companionName:profile.companionName||"",selectedTitle:selectedTitle?.titleName||"",achievementPoints:Number(profile.achievementPoints||0),score:leaderboardCategoryScore(category,scoreProfile,stats),scoreLabel:period!=="all_time"&&category==="overall"?"Period Points":leaderboardCategoryLabel(category),cosmetics:{equippedFrame:cosmetics.equippedFrame||"weathered_rope",equippedBackground:cosmetics.equippedBackground||"captains_cabin",equippedBadge:cosmetics.equippedBadge||"",showcasedAchievements:showcased}});}entries.sort((a,b)=>b.score-a.score||b.achievementPoints-a.achievementPoints||a.displayName.localeCompare(b.displayName));const label=period==="monthly"?new Intl.DateTimeFormat("en",{timeZone:"Europe/Paris",month:"long",year:"numeric"}).format(new Date()):period==="season"?configuredSeasonName(serverId):"All Time";res.set("Cache-Control","no-store");res.json({ok:true,category,period,periodKey,periodLabel:label,entries:entries.slice(0,100),cosmeticCatalog:PROFILE_COSMETICS});});

function publicRelicDisplay(relic){if(!relic||typeof relic!=="object")return null;return{name:String(relic.name||relic.id||relic.type||"Relic"),type:String(relic.type||"normal"),slot:Number(relic.slot||0),modifiers:Array.isArray(relic.modifiers)?relic.modifiers.map(value=>String(value||"")).filter(Boolean):[]};}
app.get("/profiles/public/:viewer/crew",async(req,res)=>{const scope=progressionScope(req,req.params.viewer);const key=progressionKey(scope.serverId,scope.channelId,scope.viewer);const profile=progressionProfiles.get(key);if(!profile)return res.status(404).json({ok:false,error:"Crew profile not found"});const scopedViewer=scopedViewerKey(scope.viewer,scope.channelId,scope.serverId);const requestLike={body:{},query:{serverId:scope.serverId,channelId:scope.channelId},headers:{}};const companion=profile.companionName?companionCandidatesForViewerAndName(requestLike,scopedViewer,profile.companionName)[0]||null:null;const training=profile.companionName?getTrainingState(scopedViewer,profile.companionName):null;if(!companion)return res.json({ok:true,crew:null});res.set("Cache-Control","no-store");res.json({ok:true,crew:{name:String(companion.name||profile.companionName),skin:String(companion.skin||companion.name||profile.companionName),level:Number(companion.level||1),xp:Number(companion.xp||0),neededForNextLevel:Number(companion.neededForNextLevel||0),vaultRuns:Number(companion.vaultRuns||0),temporal:String(companion.temporal||companion.temporalModifier||"None"),relics:(Array.isArray(companion.relics)?companion.relics:[]).map(publicRelicDisplay).filter(Boolean),ancientRelics:(Array.isArray(companion.ancientRelics)?companion.ancientRelics:[]).map(publicRelicDisplay).filter(Boolean),trailTypes:companion.cosmeticTrailTypes&&typeof companion.cosmeticTrailTypes==="object"?companion.cosmeticTrailTypes:{},trailColours:companion.cosmeticColours&&typeof companion.cosmeticColours==="object"?companion.cosmeticColours:{},academyLevel:Number(training?.academyLevel||1),masteryLevel:Number(training?.masteryLevel||1),sparWins:Number(training?.sparWins||0),sparLosses:Number(training?.sparLosses||0)}});});
app.post("/profile/cosmetics/equip",async(req,res)=>{const scope=progressionScope(req,req.body.viewer);const p=ensureProgression(scope,req.body.displayName||"",req.body.companionName||"");const cosmetics=ensureProfileCosmetics(p);const type=String(req.body.cosmeticType||"");const id=String(req.body.cosmeticId||"");const group=type==="frame"?"frames":type==="background"?"backgrounds":type==="badge"?"badges":"";if(!group)return res.status(400).json({ok:false,error:"Invalid cosmetic type"});if(id&&!PROFILE_COSMETICS[group].some(item=>item.id===id))return res.status(400).json({ok:false,error:"Unknown cosmetic"});if(id&&!cosmetics.unlocked[`${type}:${id}`])return res.status(403).json({ok:false,error:"That cosmetic is still locked"});if(type==="frame")cosmetics.equippedFrame=id;if(type==="background")cosmetics.equippedBackground=id;if(type==="badge")cosmetics.equippedBadge=id;cosmetics.updatedAt=new Date().toISOString();await syncProfileCosmetics(cosmetics);res.json({ok:true,cosmetics:publicProfileCosmetics(p)});});
app.post("/profile/cosmetics/showcase",async(req,res)=>{const scope=progressionScope(req,req.body.viewer);const p=ensureProgression(scope,req.body.displayName||"",req.body.companionName||"");const cosmetics=ensureProfileCosmetics(p);const ids=Array.isArray(req.body.achievementIds)?Array.from(new Set(req.body.achievementIds.map(String))):[];if(ids.length>3)return res.status(400).json({ok:false,error:"You can showcase a maximum of 3 achievements"});for(const id of ids)if(!p.achievements[id]?.unlocked)return res.status(403).json({ok:false,error:"Only unlocked achievements can be showcased"});cosmetics.showcasedAchievements=ids;cosmetics.updatedAt=new Date().toISOString();await syncProfileCosmetics(cosmetics);res.json({ok:true,cosmetics:publicProfileCosmetics(p)});});
app.post("/admin/progression/unlock-profile",requireApiKey,(req,res)=>{if(String(req.body.confirm||"").toLowerCase()!=="confirm")return res.status(400).json({ok:false,error:"Confirmation required"});const channelInput=String(req.body.channelId||req.body.channel||"");const resolved=resolveAdminViewerIdentifier(req.body.viewer,channelInput,req.body.serverId||"");if(!resolved.ok)return res.status(400).json({ok:false,error:resolved.error||"Could not resolve viewer"});const parsed=parseScopedViewerKey(resolved.viewer);const scope={serverId:normalizeServerId(resolved.serverId||parsed.serverId||firstEnabledServerId()),channelId:normalizeChannelId(resolved.channelId||parsed.channelId||resolveChannelIdInput(channelInput,resolved.serverId)),viewer:normalizeViewer(parsed.viewerId||resolved.viewer)};if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});const wallet=getWalletResolved(scopedViewerKey(scope.viewer,scope.channelId,scope.serverId),false);const linked=parseCompanionLink(wallet?.companionName||"");const p=ensureProgression(scope,wallet?.displayName||resolved.requestedViewer||scope.viewer,linked.companionName||"");const now=new Date().toISOString();let points=0;for(const def of ACHIEVEMENT_DEFINITIONS){def.targets.forEach((target,index)=>{const tier=["common","rare","epic","omega"][index];const id=`${def.id}:${tier}`;p.achievements[id]={achievementId:def.id,name:def.name,tier,progress:target,target,unlocked:true,rewardClaimed:true,unlockedAt:now};points+=[10,25,75,200][index];});if(def.title)p.titles[def.id]={titleId:def.id,titleName:def.title,sourceAchievementId:def.id,unlockedAt:now};}p.profile.achievementPoints=points;p.profile.updatedAt=now;const cosmetics=ensureProfileCosmetics(p);for(const [group,items] of Object.entries(PROFILE_COSMETICS)){const type=group==="frames"?"frame":group==="backgrounds"?"background":"badge";for(const item of items)cosmetics.unlocked[`${type}:${item.id}`]=now;}cosmetics.updatedAt=now;const result={ok:true,...scope,requestedViewer:resolved.requestedViewer,resolvedViewer:scope.viewer,achievementPoints:points,achievements:Object.keys(p.achievements).length,titles:Object.keys(p.titles).length,cosmetics:Object.keys(cosmetics.unlocked).length};res.json(result);console.log(`[PROGRESSION] Admin unlocked testing profile ${scope.viewer} @ ${scope.channelId}: ${result.achievements} achievements, ${result.titles} titles, ${result.cosmetics} cosmetics.`);Promise.all([syncProgressionProfile(p),syncAchievementBatch(p.profile,Object.values(p.achievements)),syncTitleBatch(p.profile,Object.values(p.titles)),syncProfileCosmetics(cosmetics)]).then(()=>console.log(`[PROGRESSION] Testing profile persisted for ${scope.viewer} @ ${scope.channelId}.`)).catch(error=>console.error(`[PROGRESSION] Failed persisting testing profile for ${scope.viewer} @ ${scope.channelId}:`,error));emitProgressionNotification(scope,{kind:"cosmetic",tier:"omega",title:"Testing Profile Unlocked",message:"All achievements, titles and profile cosmetics are now available.",dedupeKey:`admin_unlock_profile:${now}`,metadata:{admin:true}});});
app.post("/bounties/reroll",async(req,res)=>{const scope=progressionScope(req,req.body.viewer);const type=String(req.body.periodType||"").toLowerCase();if(!["daily","weekly"].includes(type))return res.status(400).json({ok:false,error:"Invalid bounty period"});const p=ensureProgression(scope,req.body.displayName||"",req.body.companionName||"");const state=ensureBountyState(scope,type,!!p.profile.companionName);if(state.status!=="choosing"||state.selectedBountyId)return res.status(409).json({ok:false,error:"Bounties cannot be rerolled after selecting one"});const rules=bountyRerollRules(type,state.rerollCount);if(rules.remaining<=0)return res.status(409).json({ok:false,error:`No ${type} rerolls remaining`});const scoped=scopedViewerKey(scope.viewer,scope.channelId,scope.serverId);const wallet=getWalletResolved(scoped,false)||getWallet(scoped);if(rules.nextCost>0&&Number(wallet.dirt||0)<rules.nextCost)return res.status(409).json({ok:false,error:`You need ${rules.nextCost} Dirt for this reroll`});if(rules.nextCost>0){wallet.dirt=Number(wallet.dirt||0)-rules.nextCost;wallet.updatedAt=new Date().toISOString();saveWallets();recordProgressionMetric(scope,"dirt_spent",rules.nextCost);}const nextCount=rules.used+1;const pool=type==="daily"?DAILY_BOUNTY_POOL:WEEKLY_BOUNTY_POOL;state.choices=rerollOffers(pool,`${progressionKey(scope.serverId,scope.channelId,scope.viewer)}:${state.periodKey}:reroll:${nextCount}`,!!p.profile.companionName,state.choices);state.rerollCount=nextCount;state.updatedAt=new Date().toISOString();await syncBountyState(state);emitProgressionNotification(scope,{kind:"bounty_reroll",tier:type==="weekly"?"rare":"common",title:`${type === "daily" ? "Daily" : "Weekly"} Bounties Rerolled`,message:rules.nextCost>0?`Three new choices for ${rules.nextCost} Dirt.`:"Three new choices — free reroll used.",dedupeKey:`${type}:${state.periodKey}:reroll:${nextCount}`,dedupeWindowMs:86400000});res.json({ok:true,bounty:publicBountyState(state),cost:rules.nextCost,wallet:{dirt:Number(wallet.dirt||0)}});});
...
app.post("/bounties/claim",async(req,res)=>{const scope=progressionScope(req,req.body.viewer);const type=String(req.body.periodType||"").toLowerCase();if(!["daily","weekly"].includes(type))return res.status(400).json({ok:false,error:"Invalid bounty period"});const p=ensureProgression(scope,req.body.displayName||"",req.body.companionName||"");const state=ensureBountyState(scope,type,!!p.profile.companionName);if(state.status!=="completed"||state.rewardClaimed)return res.status(409).json({ok:false,error:"Bounty is not ready to claim"});const tier=bountyRewardTier(type,state.selectedBounty);grantProgressionReward(p.profile,state.selectedBounty.reward||{},`${type}_bounty`);state.rewardClaimed=true;state.status="claimed";state.claimedAt=new Date().toISOString();emitProgressionNotification(scope,{kind:"bounty_claimed",tier,title:"Bounty Reward Claimed!",message:`Rewards from ${state.selectedBounty.name} were added to your crew.`,dedupeKey:`${type}:${state.periodKey}:claimed`,dedupeWindowMs:86400000,metadata:{periodType:type,periodKey:state.periodKey,bountyId:state.selectedBountyId,tier,reward:state.selectedBounty.reward||{}}});const metric=type==="daily"?"daily_bounties_completed":"weekly_bounties_completed";recordProgressionMetric(scope,metric,1);recordProgressionMetric(scope,"bounties_completed",1);await syncBountyState(state);try{await archiveBountyState(state);}catch(error){console.warn("[BOUNTIES] Reward claimed but history archive failed:",error?.message||error);}res.json({ok:true,bounty:publicBountyState(state),reward:state.selectedBounty.reward,tier,completedAt:state.completedAt,claimedAt:state.claimedAt});});

const SLOT_UNLOCK_LEVELS={relic:new Set([2,4,7,9]),trail:new Set([3,6,10]),ancient:new Set([10])};
app.post("/notifications/slot-unlock",(req,res)=>{const scope=progressionScope(req,req.body.viewer);const slotType=String(req.body.slotType||"").toLowerCase();const unlockLevel=Number(req.body.unlockLevel||0);const currentLevel=Number(req.body.level||0);const companionName=String(req.body.companionName||"").trim();const allowed=SLOT_UNLOCK_LEVELS[slotType];if(!scope.viewer||!scope.channelId||!companionName||!allowed?.has(unlockLevel)||currentLevel<unlockLevel)return res.status(400).json({ok:false,error:"Invalid slot unlock"});const unlockKey=`slot_unlock:${companionName.toLowerCase()}:${slotType}:${unlockLevel}`;const existing=notificationList(scope).find(notice=>notice.kind==="slot_unlock"&&String(notice.metadata?.unlockKey||"")===unlockKey);if(existing)return res.json({ok:true,created:false,notification:existing});const copy=slotType==="trail"?{title:"New Trail Slot Unlocked!",label:"Trail",tier:unlockLevel>=10?"rare":"common"}:slotType==="ancient"?{title:"Ancient Relic Slot Unlocked!",label:"Ancient Relic",tier:"epic"}:{title:"New Relic Slot Unlocked!",label:"Relic",tier:unlockLevel>=9?"rare":"common"};const notice=emitProgressionNotification(scope,{kind:"slot_unlock",tier:copy.tier,title:copy.title,message:`${companionName} unlocked a ${copy.label} slot at Level ${unlockLevel}. Click to visit the Shop.`,dedupeKey:unlockKey,dedupeWindowMs:315360000000,metadata:{unlockKey,companionName,slotType,unlockLevel,shopTarget:slotType}});res.json({ok:true,created:Boolean(notice),notification:notice});});
...
async function deleteProgressionForScope(scope){if(USE_SUPABASE){const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}`;for(const table of ["notifications","profile_cosmetics","leaderboard_rewards","leaderboard_season_stats","bounty_history","bounty_state","achievements","titles","profile_statistics","profiles"])await supabaseRequest(`/${table}?${filter}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});}const prefix=progressionKey(scope.serverId,scope.channelId,scope.viewer);for(const map of [progressionProfiles,progressionStats,progressionAchievements,progressionTitles,progressionCosmetics,progressionNotifications])map.delete(prefix);for(const key of Array.from(progressionBounties.keys()))if(key.startsWith(prefix+"::"))progressionBounties.delete(key);for(const key of Array.from(progressionSeasonStats.keys()))if(key.startsWith(prefix+"::"))progressionSeasonStats.delete(key);for(const key of Array.from(notificationDedupe.keys()))if(key.startsWith(prefix+"::"))notificationDedupe.delete(key);}
app.post("/admin/progression/reset-player",requireApiKey,async(req,res)=>{if(String(req.body.confirm||"").toLowerCase()!=="confirm")return res.status(400).json({ok:false,error:"Confirmation required"});const channelInput=String(req.body.channelId||req.body.channel||"");const resolved=resolveAdminViewerIdentifier(req.body.viewer,channelInput,req.body.serverId||"");if(!resolved.ok)return res.status(400).json({ok:false,error:resolved.error||"Could not resolve viewer"});const parsed=parseScopedViewerKey(resolved.viewer);const scope={serverId:normalizeServerId(resolved.serverId||parsed.serverId||firstEnabledServerId()),channelId:normalizeChannelId(resolved.channelId||parsed.channelId||resolveChannelIdInput(channelInput,resolved.serverId)),viewer:normalizeViewer(parsed.viewerId||resolved.viewer)};if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});await deleteProgressionForScope(scope);res.json({ok:true,reset:"profile_progression",requestedViewer:resolved.requestedViewer,resolvedViewer:scope.viewer,...scope,deleted:["achievements","titles","statistics","cosmetics","badges","bounties","bounty_history","milestones","notifications","leaderboard_rewards"]});});
app.post("/admin/bounties/reset",requireApiKey,async(req,res)=>{if(String(req.body.confirm||"").toLowerCase()!=="confirm")return res.status(400).json({ok:false,error:"Confirmation required"});const channelInput=String(req.body.channelId||req.body.channel||"");const resolved=resolveAdminViewerIdentifier(req.body.viewer,channelInput,req.body.serverId||"");if(!resolved.ok)return res.status(400).json({ok:false,error:resolved.error||"Could not resolve viewer"});const parsed=parseScopedViewerKey(resolved.viewer);const scope={serverId:normalizeServerId(resolved.serverId||parsed.serverId||firstEnabledServerId()),channelId:normalizeChannelId(resolved.channelId||parsed.channelId||resolveChannelIdInput(channelInput,resolved.serverId)),viewer:normalizeViewer(parsed.viewerId||resolved.viewer)};if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});const type=String(req.body.periodType||"all").toLowerCase();if(!["all","daily","weekly"].includes(type))return res.status(400).json({ok:false,error:"Invalid bounty period"});const filterBase=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}`;if(USE_SUPABASE)await supabaseRequest(`/bounty_state?${filterBase}${type!=="all"?`&period_type=eq.${type}`:""}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});const prefix=progressionKey(scope.serverId,scope.channelId,scope.viewer);let removed=0;for(const key of Array.from(progressionBounties.keys()))if(key.startsWith(prefix+"::")&&(type==="all"||key.endsWith(`::${type}`))){progressionBounties.delete(key);removed++;}res.json({ok:true,reset:"bounties",periodType:type,removed,requestedViewer:resolved.requestedViewer,resolvedViewer:scope.viewer,...scope});});
function rewardTier(initial,count){if(initial==="omega")return"omega";if(initial==="epic")return count>=2?"omega":"epic";return count===1?"rare":count===2?"epic":"omega";}
function leaderboardBadgeCosmeticId(rewardKey){const monthly=String(rewardKey).match(/^monthly_overall_rank_([1-5])$/);if(monthly)return`monthly_top5_rank${monthly[1]}`;const season=String(rewardKey).match(/^season_overall_rank_([1-5])$/);if(season)return`season_top5_rank${season[1]}`;return"";}
async function unlockLeaderboardBadgeCosmetic(scope,reward){const cosmeticId=leaderboardBadgeCosmeticId(reward.reward_key);if(!cosmeticId)return null;const profile=progressionProfiles.get(progressionKey(scope.serverId,scope.channelId,scope.viewer));if(!profile)return null;const p=ensureProgression(scope,profile.displayName||profile.viewer,profile.companionName||"");const cosmetics=ensureProfileCosmetics(p);const key=`badge:${cosmeticId}`;if(!cosmetics.unlocked[key]){cosmetics.unlocked[key]=new Date().toISOString();cosmetics.updatedAt=new Date().toISOString();await syncProfileCosmetics(cosmetics);emitProgressionNotification(scope,{kind:"cosmetic",tier:"rare",title:"Leaderboard Badge Unlocked!",message:String(reward.reward_name||"Top 5 Badge"),dedupeKey:`leaderboard_badge:${cosmeticId}`,dedupeWindowMs:315360000000,metadata:{cosmeticType:"badge",cosmeticId}});}const history=Array.isArray(reward.history)?reward.history:[];return{cosmeticId,count:Number(reward.award_count||history.length||1),history:history.map(item=>({periodType:item.periodType||"",periodKey:item.periodKey||"",rank:Number(item.rank||0),awardedAt:item.awardedAt||""}))};}
async function syncLeaderboardRewardBadges(scope){if(!USE_SUPABASE)return{};const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_type=eq.badge&select=*`;const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);const records={};for(const row of rows||[]){const record=await unlockLeaderboardBadgeCosmetic(scope,row);if(record)records[record.cosmeticId]=record;}return records;}
async function grantLeaderboardReward(scope,rewardKey,rewardName,initialTier,source){if(!USE_SUPABASE)return{rewardKey,count:1,tier:initialTier};const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_key=eq.${encodeURIComponent(rewardKey)}&select=*`;const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);const previous=rows?.[0]||{};const count=Number(previous.award_count||0)+1;const tier=source.fixedTier?initialTier:rewardTier(initialTier,count);const history=Array.isArray(previous.history)?previous.history:[];history.push({...source,awardedAt:new Date().toISOString(),tier});const reward={server_id:scope.serverId,channel_id:scope.channelId,viewer:scope.viewer,reward_key:rewardKey,reward_name:rewardName,reward_type:source.rewardType||"badge",award_count:count,tier,omega_count:source.fixedTier?0:tier==="omega"?Math.max(1,count-(initialTier==="rare"?2:initialTier==="epic"?1:0)):0,history,updated_at:new Date().toISOString()};await supabaseRequest("/leaderboard_rewards?on_conflict=server_id,channel_id,viewer,reward_key",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([reward])});if((source.rewardType||"badge")==="badge")await unlockLeaderboardBadgeCosmetic(scope,reward);return{rewardKey,count,tier};}
/* Rank badges deliberately override the earlier generic tier-upgrade reward writer. */
async function grantLeaderboardReward(scope,rewardKey,rewardName,initialTier,source){
    const rank=Number(source?.rank||0);
    if(rewardKey==="monthly_overall_top5"&&rank>=1&&rank<=5){rewardKey=`monthly_overall_rank_${rank}`;rewardName=`Monthly Overall #${rank} Badge`;source={...source,fixedTier:true};}
    if(rewardKey==="season_overall_top5"&&rank>=1&&rank<=5){rewardKey=`season_overall_rank_${rank}`;rewardName=`Season Overall #${rank} Badge`;source={...source,fixedTier:true};}
    if(!USE_SUPABASE)return{rewardKey,count:1,tier:initialTier};
    const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_key=eq.${encodeURIComponent(rewardKey)}&select=*`;
    const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);
    const previous=rows?.[0]||{};const count=Number(previous.award_count||0)+1;const tier=source.fixedTier?"rare":rewardTier(initialTier,count);const history=Array.isArray(previous.history)?previous.history:[];
    history.push({...source,awardedAt:new Date().toISOString(),tier});
    const reward={server_id:scope.serverId,channel_id:scope.channelId,viewer:scope.viewer,reward_key:rewardKey,reward_name:rewardName,reward_type:source.rewardType||"badge",award_count:count,tier,omega_count:source.fixedTier?0:tier==="omega"?Math.max(1,count-(initialTier==="rare"?2:initialTier==="epic"?1:0)):0,history,updated_at:new Date().toISOString()};
    await supabaseRequest("/leaderboard_rewards?on_conflict=server_id,channel_id,viewer,reward_key",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([reward])});
    if((source.rewardType||"badge")==="badge")await unlockLeaderboardBadgeCosmetic(scope,reward);
    return{rewardKey,count,tier};
}
/* Exposes badge stacks and their month/Season history to the Cosmetics UI. */
async function syncLeaderboardRewardBadges(scope){
    if(!USE_SUPABASE)return{};
    const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_type=eq.badge&select=*`;
    const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);const records={};
    for(const row of rows||[]){const record=await unlockLeaderboardBadgeCosmetic(scope,row);if(record)records[record.cosmeticId]=record;}
    const profile=progressionProfiles.get(progressionKey(scope.serverId,scope.channelId,scope.viewer));if(profile){const p=ensureProgression(scope,profile.displayName||profile.viewer,profile.companionName||"");ensureProfileCosmetics(p).leaderboardBadges=records;}
    return records;
}
app.post("/admin/progression/grant-badge",requireApiKey,async(req,res)=>{
    const channelInput=String(req.body.channelId||req.body.channel||"");
    const resolved=resolveAdminViewerIdentifier(req.body.viewer,channelInput,req.body.serverId||"");
    if(!resolved.ok)return res.status(400).json({ok:false,error:resolved.error||"Could not resolve viewer"});
    const parsed=parseScopedViewerKey(resolved.viewer);
    const scope={serverId:normalizeServerId(resolved.serverId||parsed.serverId||firstEnabledServerId()),channelId:normalizeChannelId(resolved.channelId||parsed.channelId||resolveChannelIdInput(channelInput,resolved.serverId)),viewer:normalizeViewer(parsed.viewerId||resolved.viewer)};
    if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});
    const badgeId=String(req.body.badgeId||"").trim().toLowerCase();
    const badge=PROFILE_COSMETICS.badges.find(item=>item.id===badgeId);
    if(!badge)return res.status(400).json({ok:false,error:"Unknown badge ID",validBadgeIds:PROFILE_COSMETICS.badges.map(item=>item.id)});
    const wallet=getWalletResolved(scopedViewerKey(scope.viewer,scope.channelId,scope.serverId),false);
    const linked=parseCompanionLink(wallet?.companionName||"");
    const p=ensureProgression(scope,wallet?.displayName||resolved.requestedViewer||scope.viewer,linked.companionName||"");
    const cosmetics=ensureProfileCosmetics(p);
    const rankMatch=badgeId.match(/^(monthly|season)_top5_rank([1-5])$/);
    let count=1,periodType="",periodKey="";
    if(rankMatch){
        periodType=rankMatch[1];const rank=Number(rankMatch[2]);
        periodKey=String(req.body.period||"").trim().replace(/_/g," ")||(periodType==="monthly"?monthlySeasonKey():configuredSeasonName(scope.serverId));
        if(periodType==="monthly"&&!/^\d{4}-\d{2}$/.test(periodKey))return res.status(400).json({ok:false,error:"Monthly badge period must be YYYY-MM"});
        const result=await grantLeaderboardReward(scope,`${periodType}_overall_rank_${rank}`,badge.name,"rare",{periodType,periodKey,category:"overall",rank,rewardType:"badge",fixedTier:true,admin:true});
        count=Number(result.count||1);
    }else{
        const key=`badge:${badgeId}`;const alreadyUnlocked=Boolean(cosmetics.unlocked[key]);
        cosmetics.unlocked[key]=cosmetics.unlocked[key]||new Date().toISOString();cosmetics.updatedAt=new Date().toISOString();await syncProfileCosmetics(cosmetics);
        if(!alreadyUnlocked)emitProgressionNotification(scope,{kind:"cosmetic",tier:badge.tier,title:"Badge Awarded!",message:badge.name,dedupeKey:`admin_badge:${badgeId}`,dedupeWindowMs:30000,metadata:{cosmeticType:"badge",cosmeticId:badgeId,admin:true}});
    }
    res.json({ok:true,...scope,requestedViewer:resolved.requestedViewer,resolvedViewer:scope.viewer,badgeId,badgeName:badge.name,count,periodType,periodKey});
});
async function finalizeMonthlyLeaderboard(serverId,monthKey){const categories=["overall","bounties","vaults","explorer","watchtime","academy"];let awards=0;for(const channel of streamerChannels.servers?.[serverId]?.channels||[]){const channelId=normalizeChannelId(channel.id);const rows=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(serverId,channelId,profile.viewer);rows.push({profile,stats:progressionSeasonStats.get(`${key}::${monthKey}`)?.statistics||{}});}for(const category of categories){const ranked=rows.map(row=>({...row,score:leaderboardCategoryScore(category,{achievementPoints:Number(row.stats.achievement_points||0)},row.stats)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);if(USE_SUPABASE)await supabaseRequest("/leaderboard_archives?on_conflict=server_id,channel_id,period_type,period_key,category",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:serverId,channel_id:channelId,period_type:"monthly",period_key:monthKey,category,rankings:ranked.slice(0,100).map((x,i)=>({rank:i+1,viewer:x.profile.viewer,displayName:x.profile.displayName,score:x.score})),finalized_at:new Date().toISOString()}])});if(category==="overall"){for(let i=0;i<Math.min(5,ranked.length);i++){await grantLeaderboardReward({serverId,channelId,viewer:ranked[i].profile.viewer},"monthly_overall_top5","Monthly Top 5 Badge","rare",{periodType:"monthly",periodKey:monthKey,category,rank:i+1,rewardType:"badge"});awards++;}}else if(ranked[0]){await grantLeaderboardReward({serverId,channelId,viewer:ranked[0].profile.viewer},`monthly_${category}_winner`,`${leaderboardCategoryLabel(category)} Monthly Champion`,"rare",{periodType:"monthly",periodKey:monthKey,category,rank:1,rewardType:"title"});awards++;}}}return awards;}
async function ensureMonthlyLeaderboardFinalized(serverId){const current=monthlySeasonKey();const previous=lastFinalizedLeaderboardMonths.get(serverId);if(previous===current)return;if(previous)await finalizeMonthlyLeaderboard(serverId,previous);lastFinalizedLeaderboardMonths.set(serverId,current);if(USE_SUPABASE)await supabaseRequest("/leaderboard_season_config?on_conflict=server_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:serverId,current_season_name:configuredSeasonName(serverId),last_finalized_month:current,updated_at:new Date().toISOString()}])});}
app.get("/profiles/rewards/:viewer",async(req,res)=>{const scope=progressionScope(req,req.params.viewer);if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});let rewards=[];if(USE_SUPABASE){const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&select=*&order=updated_at.desc`;rewards=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);}res.set("Cache-Control","no-store");res.json({ok:true,currentSeason:configuredSeasonName(scope.serverId),rewards});});
async function finalizeNamedSeason(serverId,seasonName){const categories=["overall","bounties","vaults","explorer","watchtime","academy"];let awards=0;for(const channel of streamerChannels.servers?.[serverId]?.channels||[]){const channelId=normalizeChannelId(channel.id);const rows=[];for(const profile of progressionProfiles.values()){if(normalizeServerId(profile.serverId)!==serverId||normalizeChannelId(profile.channelId)!==channelId)continue;const key=progressionKey(serverId,channelId,profile.viewer);rows.push({profile,stats:progressionSeasonStats.get(`${key}::season:${seasonName}`)?.statistics||{}});}for(const category of categories){const ranked=rows.map(row=>({...row,score:leaderboardCategoryScore(category,{achievementPoints:Number(row.stats.achievement_points||0)},row.stats)})).sort((a,b)=>b.score-a.score);if(USE_SUPABASE)await supabaseRequest("/leaderboard_archives?on_conflict=server_id,channel_id,period_type,period_key,category",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:serverId,channel_id:channelId,period_type:"season",period_key:seasonName,category,rankings:ranked.slice(0,100).map((x,i)=>({rank:i+1,viewer:x.profile.viewer,displayName:x.profile.displayName,score:x.score})),finalized_at:new Date().toISOString()}])});if(category==="overall"){for(let i=0;i<Math.min(5,ranked.length);i++){const scope={serverId,channelId,viewer:ranked[i].profile.viewer};await grantLeaderboardReward(scope,"season_overall_top5","Season Top 5 Badge","rare",{periodType:"season",periodKey:seasonName,category,rank:i+1,rewardType:"badge"});awards++;if(i<3){await grantLeaderboardReward(scope,"season_top3_set","Season Top 3 Cosmetic Set","epic",{periodType:"season",periodKey:seasonName,category,rank:i+1,rewardType:"cosmetic_set"});awards++;}if(i===0){await grantLeaderboardReward(scope,"season_champion_set","Season Champion Omega Set","omega",{periodType:"season",periodKey:seasonName,category,rank:1,rewardType:"cosmetic_set"});awards++;}}}else if(ranked[0]){await grantLeaderboardReward({serverId,channelId,viewer:ranked[0].profile.viewer},`season_${category}_winner`,`${leaderboardCategoryLabel(category)} Champion`,"rare",{periodType:"season",periodKey:seasonName,category,rank:1,rewardType:"title"});awards++;}}}return awards;}
app.post("/admin/leaderboard/reset-season",requireApiKey,async(req,res)=>{if(String(req.body.confirm||"").toLowerCase()!=="confirm")return res.status(400).json({ok:false,error:"Confirmation required"});const serverId=normalizeServerId(req.body.serverId||firstEnabledServerId());const nextSeason=String(req.body.nextSeason||req.body.seasonName||"").trim();if(!nextSeason)return res.status(400).json({ok:false,error:"Next Season name is required"});const current=configuredSeasonName(serverId);if(nextSeason.toLowerCase()===current.toLowerCase())return res.status(409).json({ok:false,error:"Next Season must have a different name"});const awards=await finalizeNamedSeason(serverId,current);activeLeaderboardSeasons.set(serverId,nextSeason);if(USE_SUPABASE)await supabaseRequest("/leaderboard_season_config?on_conflict=server_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:serverId,current_season_name:nextSeason,previous_season_name:current,started_at:new Date().toISOString(),updated_at:new Date().toISOString()}])});res.json({ok:true,serverId,archivedSeason:current,currentSeason:nextSeason,awardsGranted:awards,scope:"all_channels"});});
