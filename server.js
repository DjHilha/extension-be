...
const PROFILE_COSMETICS = {
    frames: [
        ...
        ,{id:"season_top3_epic",name:"Admiral's Laurel",tier:"epic",description:"Finish in the Seasonal Overall Top 3.",rewardOnly:true}
        ,{id:"season_top3_omega",name:"Eternal Admiral",tier:"omega",description:"Earn the Seasonal Top 3 set more than once.",rewardOnly:true}
        ,{id:"season_champion_omega",name:"Crown of the High Seas",tier:"omega",description:"Finish #1 Overall in a Season.",rewardOnly:true}
    ],
    backgrounds: [
        ...
        ,{id:"season_top3_epic",name:"Admiral's Quarterdeck",tier:"epic",description:"Finish in the Seasonal Overall Top 3.",rewardOnly:true}
        ,{id:"season_top3_omega",name:"Eternal Fleet",tier:"omega",description:"Earn the Seasonal Top 3 set more than once.",rewardOnly:true}
        ,{id:"season_champion_omega",name:"Throne Beyond the Maelstrom",tier:"omega",description:"Finish #1 Overall in a Season.",rewardOnly:true}
    ],
    badges: [
        ...
        ,{id:"monthly_top5_rare",name:"Monthly Silver Sextant",tier:"rare",icon:"✦",description:"Finish in the Monthly Overall Top 5.",rewardOnly:true}
        ,{id:"monthly_top5_epic",name:"Monthly Royal Sextant",tier:"epic",icon:"✧",description:"Earn the Monthly Top 5 badge twice.",rewardOnly:true}
        ,{id:"monthly_top5_omega",name:"Monthly Eternal Sextant",tier:"omega",icon:"✹",description:"Earn the Monthly Top 5 badge three or more times.",rewardOnly:true}
        ,{id:"season_top5_rare",name:"Season Iron Fleet",tier:"rare",icon:"⚔",description:"Finish in the Seasonal Overall Top 5.",rewardOnly:true}
        ,{id:"season_top5_epic",name:"Season Royal Fleet",tier:"epic",icon:"♛",description:"Earn the Seasonal Top 5 badge twice.",rewardOnly:true}
        ,{id:"season_top5_omega",name:"Season Eternal Fleet",tier:"omega",icon:"♜",description:"Earn the Seasonal Top 5 badge three or more times.",rewardOnly:true}
        ,{id:"season_top3_epic",name:"Admiral's Star",tier:"epic",icon:"★",description:"Finish in the Seasonal Overall Top 3.",rewardOnly:true}
        ,{id:"season_top3_omega",name:"Eternal Admiral's Star",tier:"omega",icon:"✺",description:"Earn the Seasonal Top 3 set more than once.",rewardOnly:true}
        ,{id:"season_champion_omega",name:"Sovereign of the Season",tier:"omega",icon:"♚",description:"Finish #1 Overall in a Season.",rewardOnly:true}
    ]
};
...
function publicBountyState(state){const rules=bountyRerollRules(state.periodType,state.rerollCount);return {...state,progress:{...(state.progress||{})},rerollsUsed:rules.used,rerollsRemaining:rules.remaining,nextRerollCost:rules.nextCost};}
...
async function unlockLeaderboardPrize(scope,rewardKey,rewardName,tier,source){
    const key=progressionKey(scope.serverId,scope.channelId,scope.viewer);const profile=progressionProfiles.get(key);if(!profile)return;
    const p=ensureProgression(scope,profile.displayName||scope.viewer,profile.companionName||"");const cosmetics=ensureProfileCosmetics(p);const now=new Date().toISOString();
    const unlock=(type,id)=>{cosmetics.unlocked[`${type}:${id}`]=now;};
    if(rewardKey==="monthly_overall_top5")unlock("badge",`monthly_top5_${tier}`);
    if(rewardKey==="season_overall_top5")unlock("badge",`season_top5_${tier}`);
    if(rewardKey==="season_top3_set"){unlock("frame",`season_top3_${tier}`);unlock("background",`season_top3_${tier}`);unlock("badge",`season_top3_${tier}`);}
    if(rewardKey==="season_champion_set"){unlock("frame","season_champion_omega");unlock("background","season_champion_omega");unlock("badge","season_champion_omega");}
    if(source.rewardType==="title"){const titleId=`leaderboard_${rewardKey}_${tier}`;p.titles[titleId]={titleId,titleName:`${rewardName} · ${tier.toUpperCase()}`,sourceAchievementId:null,unlockedAt:now};await syncTitleBatch(p.profile,[p.titles[titleId]]);}
    cosmetics.updatedAt=now;await syncProfileCosmetics(cosmetics);emitProgressionNotification(scope,{kind:"leaderboard",tier,title:"Leaderboard Reward Unlocked!",message:rewardName,dedupeKey:`leaderboard_reward:${rewardKey}:${tier}:${source.periodKey}`,metadata:{rewardKey,rewardType:source.rewardType}});
}
async function grantLeaderboardReward(scope,rewardKey,rewardName,initialTier,source){
    let count=1,tier=initialTier;
    if(USE_SUPABASE){const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}&reward_key=eq.${encodeURIComponent(rewardKey)}&select=*`;const rows=await supabaseRequest(`/leaderboard_rewards?${filter}`,{method:"GET"}).catch(()=>[]);const previous=rows?.[0]||{};count=Number(previous.award_count||0)+1;tier=rewardTier(initialTier,count);const history=Array.isArray(previous.history)?previous.history:[];history.push({...source,awardedAt:new Date().toISOString(),tier});await supabaseRequest("/leaderboard_rewards?on_conflict=server_id,channel_id,viewer,reward_key",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=minimal"},body:JSON.stringify([{server_id:scope.serverId,channel_id:scope.channelId,viewer:scope.viewer,reward_key:rewardKey,reward_name:rewardName,reward_type:source.rewardType||"badge",award_count:count,tier,omega_count:tier==="omega"?Math.max(1,count-(initialTier==="rare"?2:initialTier==="epic"?1:0)):0,history,updated_at:new Date().toISOString()}])});}
    await unlockLeaderboardPrize(scope,rewardKey,rewardName,tier,source);return{rewardKey,count,tier};
}
