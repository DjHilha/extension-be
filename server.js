...
app.post("/admin/progression/unlock-profile",requireApiKey,async(req,res)=>{const scope=progressionScope(req,req.body.viewer);if(String(req.body.confirm||"").toLowerCase()!=="confirm")return res.status(400).json({ok:false,error:"Confirmation required"});if(!scope.viewer||!scope.channelId)return res.status(400).json({ok:false,error:"Missing viewer or channel"});const wallet=getWalletResolved(scopedViewerKey(scope.viewer,scope.channelId,scope.serverId),false);const linked=parseCompanionLink(wallet?.companionName||"");const p=ensureProgression(scope,wallet?.displayName||scope.viewer,linked.companionName||"");const now=new Date().toISOString();let points=0;for(const def of ACHIEVEMENT_DEFINITIONS){def.targets.forEach((target,index)=>{const tier=["common","rare","epic","omega"][index];const id=`${def.id}:${tier}`;p.achievements[id]={achievementId:def.id,name:def.name,tier,progress:target,target,unlocked:true,rewardClaimed:true,unlockedAt:now};points+=[10,25,75,200][index];});if(def.title)p.titles[def.id]={titleId:def.id,titleName:def.title,sourceAchievementId:def.id,unlockedAt:now};}p.profile.achievementPoints=points;p.profile.updatedAt=now;const cosmetics=ensureProfileCosmetics(p);for(const [group,items] of Object.entries(PROFILE_COSMETICS)){const type=group==="frames"?"frame":group==="backgrounds"?"background":"badge";for(const item of items)cosmetics.unlocked[`${type}:${item.id}`]=now;}cosmetics.updatedAt=now;await Promise.all([syncProgressionProfile(p),syncAchievementBatch(p.profile,Object.values(p.achievements)),syncTitleBatch(p.profile,Object.values(p.titles)),syncProfileCosmetics(cosmetics)]);emitProgressionNotification(scope,{kind:"cosmetic",tier:"omega",title:"Testing Profile Unlocked",message:"All achievements, titles and profile cosmetics are now available.",dedupeKey:`admin_unlock_profile:${now}`,metadata:{admin:true}});res.json({ok:true,...scope,achievementPoints:points,achievements:Object.keys(p.achievements).length,titles:Object.keys(p.titles).length,cosmetics:Object.keys(cosmetics.unlocked).length});});
