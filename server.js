...
function recordProgressionMetric(scope, metric, amount=1, meta={}) {
    ...
    evaluateAchievements(p);ensureProfileCosmetics(p);syncProgressionProfile(p).catch(()=>{});
}
...
async function loadProgressionFromSupabase(){
    if(!USE_SUPABASE)return;
    try{
        const [profiles,stats,bounties,achievements,titles,notifications,cosmetics]=await Promise.all([
...
async function deleteProgressionForScope(scope){if(USE_SUPABASE){const filter=`server_id=eq.${encodeURIComponent(scope.serverId)}&channel_id=eq.${encodeURIComponent(scope.channelId)}&viewer=eq.${encodeURIComponent(scope.viewer)}`;for(const table of ["notifications","profile_cosmetics","bounty_history","bounty_state","achievements","titles","profile_statistics","profiles"])await supabaseRequest(`/${table}?${filter}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});}const prefix=progressionKey(scope.serverId,scope.channelId,scope.viewer);for(const map of [progressionProfiles,progressionStats,progressionAchievements,progressionTitles,progressionCosmetics,progressionNotifications])map.delete(prefix);for(const key of Array.from(progressionBounties.keys()))if(key.startsWith(prefix+"::"))progressionBounties.delete(key);}
